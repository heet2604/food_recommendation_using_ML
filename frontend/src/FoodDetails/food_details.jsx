...
function FoodDetails() {
  ...
  const fetchFoodData = useCallback(async () => {
    try {
      ...
      if (location.state && location.state.foodId) {
        response = await axios.get(`ec2-3-110-83-161.ap-south-1.compute.amazonaws.com/api/selected-food/${location.state.foodId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        response = await axios.get(`ec2-3-110-83-161.ap-south-1.compute.amazonaws.com/api/latest-food/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
  ...
  const sendDataToDashboard = async () => {
    try {
      ...
      await axios.post(
        "ec2-3-110-83-161.ap-south-1.compute.amazonaws.com/api/add-food-to-dashboard",
        nutritionalData,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
