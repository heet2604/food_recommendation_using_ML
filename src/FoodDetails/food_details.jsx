...
function FoodDetails() {
  ...
  const navigate = useNavigate();
    ...
    const sendDataToDashboard = async () => {
        try {
            const adjustedEnergy = updatedValues(foodData.energy_kcal);
            const adjustedProtein = updatedValues(foodData.protein_g);
            const adjustedCarbs = updatedValues(foodData.carb_g);
            const adjustedFats = updatedValues(foodData.fat_g);
            const adjustedFiber = updatedValues(foodData.fibre_g);

            const response = await axios.post(
                "http://localhost:5000/api/add-food-to-dashboard",
                {
                    energy_kcal: adjustedEnergy,
                    protein_g: adjustedProtein,
                    carb_g: adjustedCarbs,
                    fat_g: adjustedFats,
                    fibre_g: adjustedFiber,
                },
                {
                    headers: {
            ...
            navigate("/home")
            console.log("Data sent to dashboard:", response.data);
        } catch (error) {
    ...
    <div className="bg-black text-white min-h-screen flex flex-col">
        ...
        {isMenuOpen && (
            <div className="w-full bg-gray-800 px-6 py-4 flex flex-col items-center lg:hidden fixed top-16 left-0 z-50">
                ...
                <a href="/premium" className="cursor-pointer hover:text-gray-400 py-2">
                    Explore Premium
                </a>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-center p-5">
            ...
            <div className="rounded-lg outline text-white w-96 text-center p-5 mt-5 shadow-lg bg-gray-800 max-w-xs ">
                ...
                <button
                    ...
                    className="bg-green-500 text-white py-2 px-4 rounded-lg mt-4 hover:bg-green-700 transition"
